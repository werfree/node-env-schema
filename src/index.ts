...
export function validateEnv<S extends EnvSchema>(schema: S): EnvResult<S> {
  ...
  if (process === undefined || process.env === undefined || typeof process.env !== 'object') {
    throw new Error('process or process.env is not defined');
  }
  ...
  for (const key in schema) {
    ...
    if (expectedType === 'string') {
      ...
      if (value === 'true') env[key] = true;
        ...
        throw new Error(
          `Expected environment variable ${key} to be boolean (true/false), but got "${value}"`,
        );
      ...
      try {
        ...
        throw new Error(
          `Expected environment variable ${key} to be a valid URL, but got "${value}"`,
        );
      }
      ...
      if (!uriPattern.test(value)) {
        throw new Error(
          `Expected environment variable ${key} to be a valid URI, but got "${value}"`,
        );
      }
      env[key] = value;
      ...
      if (isNaN(port) || port < 0 || port > 65535 || !Number.isInteger(port)) {
        throw new Error(
          `Expected environment variable ${key} to be a valid port number (0-65535), but got "${value}"`,
        );
      }
      env[key] = port;
      ...
      if (!expectedType.includes(value)) {
        throw new Error(
          `Expected environment variable ${key} to be one of [${expectedType.join(', ')}], but got "${value}"`,
        );
      }
      env[key] = value;
    } else {
